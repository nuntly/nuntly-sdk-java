...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { NuntlyInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw NuntlyInvalidDataException("`$name` is not set")
            is JsonNull -> throw NuntlyInvalidDataException("`$name` is null")
            else -> throw NuntlyInvalidDataException("`$name` is invalid, received $this")
        }
