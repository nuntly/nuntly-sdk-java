...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(webhookCreateResponse: WebhookCreateResponse) = apply {
            ...
            endpointUrl = webhookCreateResponse.endpointUrl
            events = webhookCreateResponse.events.map { it.toMutableList() }
            kind = webhookCreateResponse.kind
            orgId = webhookCreateResponse.orgId
            region = webhookCreateResponse.region
            signingSecret = webhookCreateResponse.signingSecret
            status = webhookCreateResponse.status
            modifiedAt = webhookCreateResponse.modifiedAt
    ...
    override fun toString() =
        "WebhookCreateResponse{id=$id, createdAt=$createdAt, endpointUrl=$endpointUrl, events=$events, kind=$kind, orgId=$orgId, region=$region, signingSecret=$signingSecret, status=$status, modifiedAt=$modifiedAt, name=$name, additionalProperties=$additionalProperties}"
}
