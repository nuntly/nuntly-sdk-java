...
private constructor(
    ...
    private constructor(
        ...
        private val send: JsonField<SendDetail>,
        private val additionalProperties: MutableMap<String, JsonValue>,
    ) {
        ...
        private constructor(
            ...
            @JsonProperty("send") @ExcludeMissing send: JsonField<SendDetail> = JsonMissing.of(),
        ) : this(
            id,
            domain,
            domainId,
         ...
         *   unexpectedly missing or null (e.g. if the server responded with an unexpected value).
         */
        fun send(): SendDetail = send.getRequired("send")
        ...
        /**
         * Returns the raw JSON value of [send].
         *
         * Unlike [send], this method doesn't throw if the JSON field has an unexpected type.
         */
        @JsonProperty("send") @ExcludeMissing fun _send(): JsonField<SendDetail> = send
        ...
        class Builder internal constructor() {
            ...
            private var send: JsonField<SendDetail>? = null
            private var additionalProperties: MutableMap<String, JsonValue> = mutableMapOf()
            ...
            fun send(send: SendDetail) = send(JsonField.of(send))

            /**
             * Sets [Builder.send] to an arbitrary JSON value.
             *
             * You should usually call [Builder.send] with a well-typed [SendDetail] value instead.
             * This method is primarily for setting the field to an undocumented or not yet
             * supported value.
             */
            fun send(send: JsonField<SendDetail>) = apply { this.send = send }
        ...
        override fun toString() =
            "Data{id=$id, domain=$domain, domainId=$domainId, enqueueAt=$enqueueAt, from=$from, messageId=$messageId, orgId=$orgId, sentAt=$sentAt, subject=$subject, to=$to, bcc=$bcc, bulkId=$bulkId, cc=$cc, headers=$headers, replyTo=$replyTo, tags=$tags, send=$send, additionalProperties=$additionalProperties}"
    }
    ...
    override fun toString() =
        "EmailSentEvent{id=$id, createdAt=$createdAt, type=$type, kind=$kind, data=$data, additionalProperties=$additionalProperties}"
}
